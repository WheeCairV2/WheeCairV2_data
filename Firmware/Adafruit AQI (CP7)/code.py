...
wifi = adafruit_esp32spi_wifimanager.ESPSPI_WiFiManager(esp, secrets, status_pixel=None, attempts=4, connection_type=1, debug=False)

# Connect to a PM2.5 sensor over UART
reset_pin = None
uart = busio.UART(board.TX3, board.RX3, baudrate=9600,  timeout=60)
...
def sample_aq_sensor():
    ...
    # sample pm2.5 sensor over 3 sec sample rate
    while time.monotonic() - time_start <= 3:
        try:
            aqdata = pm25.read()
